...
import { Button, Stack, Typography, MenuItem, IconButton, styled } from "@mui/material";
...
const InformationThree = (props) => {
    ...
    const { fields: jobExperienceFields, append: jobExperienceAppend, remove: jobExperienceRemove, replace: jobExperienceReplace} = useFieldArray({ control, name: "jobExperience", });
    const { fields: educationFields, append: educationAppend, remove: educationRemove, replace: educationReplace} = useFieldArray({ control, name: "education", });
        ...
        <Fragment>
            <div>
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <Button className="btn-addExperience" startIcon={<i className="fa-solid fa-plus"></i>} variant="outlined" style={{paddingLeft: 32, paddingRight: 32}} onClick={()=>{jobExperienceAppend({experience: "", year: ""})}}>เพิ่มประสบการณ์</Button>
                </Stack>
                {jobExperienceFields.map((item, index) => (
                    <StyledExperience key={item.id}>
                        ...
                        />
                        <IconButton style={{width: 40, height: 40}} onClick={()=>{jobExperienceRemove(index)}}><i className="fa-solid fa-trash-can"></i></IconButton>
                    </StyledExperience>
                ))}
            </div>
            ...
            <div style={{marginTop: 48}}>
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <Button className="btn-addEducation" startIcon={<i className="fa-solid fa-plus"></i>} variant="outlined" style={{paddingLeft: 32, paddingRight: 32}} onClick={()=>{educationAppend({detail: "", year: ""})}}>เพิ่มประวัติ</Button>
                </Stack>
                {educationFields.map((item, index) => (
                    <StyledExperience key={item.id}>
                        ...
                        />
                        <IconButton style={{width: 40, height: 40}} onClick={()=>{educationRemove(index)}}><i className="fa-solid fa-trash-can"></i></IconButton>
                    </StyledExperience>
                ))}
            </div>
        </Fragment>
