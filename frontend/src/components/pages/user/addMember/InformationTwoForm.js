...
const StyledSpecialization = styled("div")(({ selected }) => ({
  ...
  transition:
    "border 300ms cubic-bezier(0.4, 0, 0.2, 1) 0ms, background-color 300ms cubic-bezier(0.4, 0, 0.2, 1) 0ms",
  borderRadius: 20,
  userSelect: "none",
  "& .MuiTypography-root": {
...
const InformationTwo = (props) => {
  ...
  const {
    fields: jobSkillFields,
    append: jobSkillAppend,
    remove: jobSkillRemove,
  } = useFieldArray({ control, name: "jobSkill" });
  ...
  return (
    <Fragment>
          ...
          render={(field) => (
            <>
              {jobSpecialization.map((specialization) => (
                <Grid
                  key={`specialization_${specialization}`}
                  item
                  xs={6}
                  sm={4}
                >
                  <StyledSpecialization
                    ...
                    selected={specialization === getValues("jobSpecialization")}
                  >
      ...
      {jobSpecializationWatched && (
        <div style={{ marginTop: 48 }}>
          <Typography gutterBottom style={{ color: "#9e9e9e" }}>
            เลือก 3 บทบาทที่บอกถึงพนักงานคนนี้
          </Typography>
          <Grid container spacing={1} alignItems="start">
            <Controller
              name="jobRole"
                  ...
                  {jobRole[getValues("jobSpecialization")]?.map((role) => (
                    <Grid key={`role_${role}`} item xs={6} sm={4}>
                      <StyledRole
                        ...
                        selected={getValues("jobRole").includes(role)}
                      >
